...
for (let item of input) {
    //First we split each element in the array in two different ones
    let splitted = item.split(':');
    let policy = splitted[0];
    let password = splitted[1].trim();
    
    //Then we define the start and the end of the range and cast them into integers
    let ranges = policy.split(' ')[0];
    let startNumber = Number(ranges.split('-')[0]);
    let endNumber = Number(ranges.split('-')[1]);

    let letter = policy.split(' ')[1];
    let splittedPassword = password.split('');
     
    if (splittedPassword[startNumber -1] === letter && splittedPassword[endNumber - 1] !== letter) valid++; 
    else if (splittedPassword[endNumber - 1] === letter && splittedPassword[startNumber - 1] !== letter) valid++;
}
